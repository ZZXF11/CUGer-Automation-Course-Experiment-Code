...
extern char Image$$ZI$$Base[];
	...
	extern unsigned char cameramini[];
...
void display_menu(void){
	...
	Paint_Bmp(27,170,105,60,drummini);//项目1
	Paint_Bmp(187,170,105,60,keyboardmini);//项目2
	Paint_Bmp(347,170,105,60,bassmini);//项目3
	Paint_Bmp(220,78,40,35,cameramini);//项目3

}


void display_time(int secondSHI,int secondGE,int minuteSHI,int minuteGE)//显示时间
{
	Lcd_PutASCII(328,0,'2',0xffff,0x0000,0);//2021.12.20.10:00
	Lcd_PutASCII(336,0,'0',0xffff,0x0000,0);
	Lcd_PutASCII(344,0,'2',0xffff,0x0000,0);
	Lcd_PutASCII(352,0,'1',0xffff,0x0000,0);
	Lcd_PutASCII(360,0,'/',0xffff,0x0000,0);
	Lcd_PutASCII(368,0,'1',0xffff,0x0000,0);
	Lcd_PutASCII(376,0,'2',0xffff,0x0000,0);
	Lcd_PutASCII(384,0,'/',0xffff,0x0000,0);
	Lcd_PutASCII(392,0,'2',0xffff,0x0000,0);
	Lcd_PutASCII(400,0,'0',0xffff,0x0000,0);
	Lcd_PutASCII(408,0,' ',0xffff,0x0000,0);
	Lcd_PutASCII(416,0,'1',0xffff,0x0000,0);
	Lcd_PutASCII(424,0,'0',0xffff,0x0000,0);
	Lcd_PutASCII(432,0,':',0xffff,0x0000,0);
	Lcd_PutASCII(440,0,minuteSHI,0xffff,0x0000,0);
	Lcd_PutASCII(448,0,minuteGE,0xffff,0x0000,0);
	//Lcd_PutASCII(456,0,':',0xffff,0x0000,0);
	//Lcd_PutASCII(464,0,secondSHI,0xffff,0x0000,0);
	//Lcd_PutASCII(472,0,secondGE,0xffff,0x0000,0);
}
...
void select_irq(void){
		...
		{
			if(xdata>27 && xdata<132)//如果点击项目1
			{
				Paint_Bmp(0, 0, 480, 272, drum);
				flag_irq=2;
			}
			else if(xdata>187 && xdata<292)//如果点击项目2
			{
				
				Paint_Bmp(0, 0, 480, 272, keyboard);
				flag_irq=1;
			}
			else if(xdata>347 && xdata<452)//如果点击项目3
			{
		...
		else if(xdata>220 && xdata<260 && ydata>78 && ydata<113){
				flag_irq=4;	
		}
...
void play_the_drum(void){
	...
	else if(ydata>80 && ydata<115 && xdata>24 && xdata<130){Play_drums(1);}
	else if(ydata>154 && ydata<182 && xdata>38 && xdata<130){Play_drums(2);}
	else if(ydata>194 && ydata<248 && xdata>184 && xdata<296){Play_drums(3);}
	else if(ydata>194 && ydata<248 && xdata>75 && xdata<160){ Play_drums(4);}
	else if(ydata>194 && ydata<248 && xdata>310 && xdata<422){ Play_drums(5);}//*********************
	else if(ydata>115 && ydata<154 && xdata>166 && xdata<220){ Play_drums(6);}
	else if(ydata>115 && ydata<154 && xdata>276 && xdata<328){ Play_drums(7);}
	else if(ydata>115 && ydata<154 && xdata>400 && xdata<440){ Play_drums(8);}//*********************
}
