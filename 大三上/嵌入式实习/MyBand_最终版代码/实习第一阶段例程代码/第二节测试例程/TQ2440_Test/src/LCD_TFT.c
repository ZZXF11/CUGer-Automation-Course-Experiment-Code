...
void Lcd_TFT_Test( void ) 
{
   	...
   	Paint_Bmp(0, 0, 480, 272, xiYY);
   	Uart_Printf( "\nDisplay Black! Press any key to continue!\n" );
	Uart_Getch() ;		//wait uart input
	
	Paint_Bmp(0, 0, 480, 272, meiYY);
   	Uart_Printf( "\nDisplay Black! Press any key to continue!\n" );
	Uart_Getch() ;		//wait uart input

	Paint_Bmp(0, 0, 480, 272, manYY_2);
   	Uart_Printf( "\nDisplay Black! Press any key to continue!\n" );
	Uart_Getch() ;		//wait uart input
	
	Paint_Bmp(0, 0, 480, 272, feiYY);
   	Uart_Printf( "\nDisplay Black! Press any key to continue!\n" );
	Uart_Getch() ;		//wait uart input

	Paint_Bmp(0, 0, 480, 272, lanYY);
   	Uart_Printf( "\nDisplay Black! Press any key to continue!\n" );
	Uart_Getch() ;		//wait uart input;
	
	Paint_Bmp(0, 0, 480, 272, nuanYY);
   	Uart_Printf( "\nDisplay Black! Press any key to continue!\n" );
	Uart_Getch() ;		//wait uart input
...
void Lcd_TFT_Test_3( void ) {
	...
	while(Uart_GetKey() != ESC_KEY)
	{
		for(ii=0;ii<32;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(ii<<11)|(0<<5)|(0));
		}
		for(ii=32;ii<95;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0<<11)|(ii-31)<<5|(0));
		}
		for(ii=95;ii<126;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0<<11)|(0<<5)|(ii-96));
		}
		for(ii=126;ii<((pix_x)-126)/13+126;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0x1f<<11)|(0<<5)|(0x1f));
		}
		for(;ii<((pix_x)-126)*2/13+126;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0<<11)|(0<<5)|(0x1f));
		}
		for(;ii<((pix_x)-126)*3/13+126;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0<<11)|(0x3f<<5)|(0x1f));
		}
		for(;ii<((pix_x)-126)*4/13+126;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0<<11)|(0x3f<<5)|(0));
		}
		for(;ii<((pix_x)-126)*5/13+126;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0x1f<<11)|(0x3f<<5)|(0));
		}
		for(;ii<((pix_x)-126)*6/13+126;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0x1f<<11)|(0<<5)|(0x1f));
		}
		for(;ii<((pix_x)-126)*7/13+126;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0x0f<<11)|(0<<5)|(0x0f));
		}
		for(;ii<((pix_x)-126)*8/13+126;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0<<11)|(0<<5)|(0x0f));
		}
		for(;ii<((pix_x)-126)*9/13+126;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0<<11)|(0x1f<<5)|(0x0f));
		}
		for(;ii<((pix_x)-126)*10/13+126;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0<<11)|(0x1f<<5)|(0));
		}
		for(;ii<((pix_x)-126)*11/13+126;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0x0f<<11)|(0x1f<<5)|(0));
		}
		for(;ii<((pix_x)-126)*12/13+126;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0x0f<<11)|(0<<5)|(0));
		}
		for(;ii<pix_x;ii++)
		{
			Glib_Line(ii,0,ii,pix_y,(0<<11)|(0<<5)|(0));
		}
	}
