...
void Init()  // ******初始化程序******
{
  ...
  Command = 0xa0;//显示方向：反向
  WriteCommandE1();
  WriteCommandE2();

  Command = 0xc0;//设置显示起始行(0~32),行地址计数器循环计数
  WriteCommandE1();
  WriteCommandE2();
...
void Display_1(int SDpeed)	//显示手动
{  
  Init();        //初始化  (移到主函数)
  Clear();       //清屏
...
void Display_4(int indexR1,int indexR2,int indexR3,int indexAng1,int indexAng2,int indexAng3)//画圆
{  
  Init();        //初始化  (移到主函数)
  Clear();       //清屏
