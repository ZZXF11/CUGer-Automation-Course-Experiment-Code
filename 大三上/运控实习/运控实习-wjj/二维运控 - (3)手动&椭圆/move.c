...
//	{
//		if(direction == '+'){//判断方向，给方向位赋值
//		PortA=0xf0;				//启动电机让其运动一个机器周期（即实现走一步的功能）
//		for (i =0; i < speed; i++) _nop_();
//		PortA=0xff;
//		for (i =0; i < speed; i++) _nop_();
//		PortA=0xf0;
//		}
//		else{ //如果-向
//		PortA=0x00;				//启动电机让其运动一个机器周期（即实现走一步的功能）
//		for (i =0; i < speed; i++) _nop_();
//		PortA=0x0f;
//		for (i =0; i < speed; i++) _nop_();
//		PortA=0x00;
...
//	{
//		if(direction == '+'){//判断方向，给方向位赋值
//		PortB=0xf0;				//启动电机让其运动一个机器周期（即实现走一步的功能）
//		for (i =0; i < speed; i++) _nop_();
//		PortB=0xff;
//		for (i =0; i < speed; i++) _nop_();
//		PortB=0xf0;
//		}
//		else{ 
//		PortB=0x00;				//启动电机让其运动一个机器周期（即实现走一步的功能）
//		for (i =0; i < speed; i++) _nop_();
//		PortB=0x0f;
//		for (i =0; i < speed; i++) _nop_();
//		PortB=0x00;
			...
			if(noTuiChu==0) break;
						...
						switch (keynum)  //keynum就等于按键的数值
						{
								 ...
								 case 0x0b: {
									 ...
									 move_onestep('y','+',SDspeed);
								 break;}
								 case 0x0c: {SDspeed-=1;Display_1(SDspeed);do{Delaykey(10);}while((keyscan2())==0x0c);break;}//c键加速
								 case 0x08: {SDspeed+=1;Display_1(SDspeed);do{Delaykey(10);}while((keyscan2())==0x08);break;}//8键减速								 
