...
void xzz(int speed1){
  ...
  if(mark == 5){
	...
	if(mark == 7){
	printf("page6.t5.txt=\"%ld\"\xff\xff\xff",current_x);}
	
	if(mark == 6){
	printf("page5.t5.txt=\"%ld\"\xff\xff\xff",current_x);}
	if(mark == 8){
	printf("page7.t5.txt=\"%ld\"\xff\xff\xff",current_x);}
}
...
void yfz(int speed1){
  ...
  DAL2 = 0;
	...
	if(mark == 7){
	printf("page6.t7.txt=\"%ld\"\xff\xff\xff",current_y);}
	
	
	if(mark == 6){
	printf("page5.t7.txt=\"%ld\"\xff\xff\xff",current_y);}
	if(mark == 8){
	printf("page7.t7.txt=\"%ld\"\xff\xff\xff",current_y);}
...
int zhixianchabu1(long x,long y){//输入x，y为目标位置
	int xm=0,ym=0;//当前位置与初始位置差
	int xe,ye;//判断是否到达
...
{	
	...
	zhixianchabu1(150,0);
	delay_ms(5000);
	zhixianchabu1(151,30);
	delay_ms(5000);
	zhixianchabu1(110,30);
	delay_ms(5000);
	yuanhuchabu(80,12,50,30,34,0);
	delay_ms(5000);
	zhixianchabu1(0,15);
	delay_ms(5000);
	yuanhuchabu(50,8,0,0,50,0);
	delay_ms(5000);
	return;
}
...
void sanjiao(double A,double w,double b){
  double t=0;
	double xm=0,ym=0;
	double xe1=-1;
	double ye1=-1;//存放之前的坐标
	double flag=1;//判断斜率
	double ye=b;
	double xm1=xm+1;
	t=2*3.14/w;
	...
	while(xm<t){
	//xm=0;//当前x
	ym=A*sin(w*xm)+b;//对应的正弦值
	flag=cos(w*xm);
	//ye=current_y;//当前y
	//flag=(ye-ye1)*(xm-xe1);//判断斜率
		if(supercase==0){
			supercase = 1;
		  break;
		}
...
void sanjiaocos(double A,double w,double b){
	double t=0;
	double xm=0,ym=0;
	double xe1=-1;
	double ye1=-1;//存放之前的坐标
